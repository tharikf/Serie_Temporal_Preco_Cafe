...
def retomando_valor_previsto(dataframe_original, dataframe_diferenciado):
    ...
    for data, valor_original, previsao_diferenciada in zip(dataframe_diferenciado.index, dataframe_original.iloc[1:, ], dataframe_diferenciado):
        valor_previsto = round((valor_original + previsao_diferenciada), 2)
        lista_vazia.append([data, valor_original, valor_previsto])
